...
import { updatePairDayData, updateTokenDayData, updateCrowDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  let crow = CrowFactory.load(FACTORY_ADDRESS);

  // reset factory liquidity by subtracting only tracked liquidity
  crow.totalLiquidityBNB = crow.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  crow.totalLiquidityBNB = crow.totalLiquidityBNB.plus(trackedLiquidityBNB);
  crow.totalLiquidityUSD = crow.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  crow.totalTransactions = crow.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let crow = CrowFactory.load(FACTORY_ADDRESS);
  crow.totalVolumeUSD = crow.totalVolumeUSD.plus(trackedAmountUSD);
  crow.totalVolumeBNB = crow.totalVolumeBNB.plus(trackedAmountBNB);
  crow.untrackedVolumeUSD = crow.untrackedVolumeUSD.plus(derivedAmountUSD);
  crow.totalTransactions = crow.totalTransactions.plus(ONE_BI);
  ...
  let crowDayData = updateCrowDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  crowDayData.dailyVolumeUSD = crowDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  crowDayData.dailyVolumeBNB = crowDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  crowDayData.dailyVolumeUntracked = crowDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  crowDayData.save();
